...
<template>
  <Teleport to="body">
    <Transition name="ff-modal">
      <div v-show="modelValue" class="ff-overlay" role="presentation" @click.self="onMaskClick">
        <div class="ff-modal" :class="`ff-modal--${size}`" role="dialog" aria-modal="true">
          <div v-if="title || closable" class="ff-modal__header">
            <h3 v-if="title" class="ff-modal__title">{{ title }}</h3>
