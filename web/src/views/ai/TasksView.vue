...
.tv__tabs { display: flex; gap: 4px; background: var(--ff-bg-subtle, #f1f4f2); border-radius: 10px; padding: 3px; }
.tv__tab { border: none; background: none; border-radius: 8px; padding: 6px 15px; font-size: 13px; font-weight: 600; color: var(--ff-text-2, #6b7280); cursor: pointer; }
.tv__tab.on { background: var(--ff-bg-surface, #fff); color: var(--ff-brand-dark, #1d4e39); box-shadow: 0 1px 3px rgba(16, 40, 30, 0.12); }
.tv__sp { flex: 1; }
.tv__retry-all { display: inline-flex; align-items: center; gap: 6px; border: 1px solid #f5c6c8; background: var(--ff-up-subtle, #fdecec); color: var(--ff-down, #e5484d); border-radius: 9px; padding: 7px 13px; font-size: 12.5px; font-weight: 600; cursor: pointer; }
.tv__retry-all:disabled { opacity: 0.5; }
.tv__list { display: flex; flex-direction: column; gap: 10px; }
.tv__item { background: var(--ff-bg-surface, #fff); border: 1px solid var(--ff-border, #e5e7eb); border-radius: 12px; padding: 14px 16px; }
...
.tv__badge.ok { background: var(--ff-down-subtle, #e8f7ee); color: var(--ff-up, #12a150); }
.tv__badge.bad { background: var(--ff-up-subtle, #fdecec); color: var(--ff-down, #e5484d); }
.tv__badge.run { background: var(--ff-bg-brand-subtle, #eaf4ef); color: var(--ff-brand-dark, #1d4e39); }
.tv__badge.idle { background: var(--ff-bg-subtle, #f1f4f2); color: var(--ff-text-3, #9ca3af); }
.tv__dot { width: 7px; height: 7px; border-radius: 50%; }
.tv__dot.ok { background: var(--ff-up, #12a150); }
.tv__dot.bad { background: var(--ff-down, #e5484d); }
.tv__dot.run { background: var(--ff-brand, #2f7d5b); animation: tv-pulse 1.2s infinite; }
.tv__dot.idle { background: var(--ff-text-3, #9ca3af); }
@keyframes tv-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }
.tv__name { font-size: 13.5px; font-weight: 600; color: var(--ff-text-primary, #1f2937); }
...
.tv__errmsg { font-size: 12.5px; color: var(--ff-down, #e5484d); }
.tv__ops { display: flex; gap: 6px; }
.tv__op { display: inline-flex; align-items: center; gap: 5px; border: 1px solid var(--ff-border, #e5e7eb); background: var(--ff-bg-surface, #fff); border-radius: 8px; padding: 6px 12px; font-size: 12px; font-weight: 600; color: var(--ff-text-2, #6b7280); cursor: pointer; }
.tv__op:hover { border-color: var(--ff-border-brand, #9fc3b1); color: var(--ff-brand, #2f7d5b); }
.tv__op--danger { color: var(--ff-down, #e5484d); border-color: #f5c6c8; }
.tv__op--link { color: var(--ff-brand, #2f7d5b); border-color: var(--ff-border-brand, #bfd9cc); }
.tv__empty { text-align: center; padding: 60px 20px; color: var(--ff-text-3, #9ca3af); background: var(--ff-bg-surface, #fff); border: 1px dashed var(--ff-border, #d8dfdb); border-radius: 13px; }
...
.tv__log { width: 420px; max-width: 90vw; height: 100%; background: var(--ff-bg-surface, #fff); padding: 16px; box-shadow: -8px 0 24px rgba(10, 30, 22, 0.15); }
...
.tv__log-x { border: none; background: var(--ff-bg-subtle, #f3f6f4); border-radius: 8px; width: 30px; height: 30px; cursor: pointer; color: var(--ff-text-2, #6b7280); }
.tv__log-kv { display: flex; gap: 12px; padding: 8px 0; border-bottom: 1px dashed var(--ff-border, #eef1f0); font-size: 13px; }
.tv__log-kv span { color: var(--ff-text-3, #9ca3af); width: 48px; flex-shrink: 0; }
.tv__log-kv b { color: var(--ff-text-primary, #1f2937); word-break: break-all; font-weight: 600; }
