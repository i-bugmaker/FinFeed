...
- **结论（2026-08-17 已修复）：9 类目全部接入。其中 8 类返回真实数据（热股 / 板块 / ETF / 热门 / 可转债 / 港美-港股 / 热基 / 期货）；保险以同花顺专区外链降级；港美-美股子榜需登录同花顺账号（已明确提示）。**
...
| 港美 | `hk-us` | `stock?stock_type=hk`（港股 ✅；`stock_type=us` 美股需登录） | ✅(港股)/❌(美股) | ✅(港股) / 美股已提示 |
| 保险 | `insurance` | amis/Kamis 第三方嵌入（`#insurance`, systemId 171, token） | ❌ | 无公开 JSON，外链降级（已上线） |
...
| **Tier-3** | 保险 | 第三方 amis 嵌入，无公开热榜 JSON | 无 | 极高（建议暂缓/外链） |
...
- 渲染分两类：
  - **证券型**（热股/板块/可转债/期货/ETF/热基/港美）：沿用现有「排名 + 名称/代码 + 热度条 + 涨跌幅（红涨绿跌）」布局，板块追加 ETF 联动标签，期货可折叠关联个股。
  - **内容型**（热门）：「排名 + 话题标题 + 热度 + 摘要 + 跳转」。
- 占位文案仅保留给尚未接入的 Tier-3（保险）。
