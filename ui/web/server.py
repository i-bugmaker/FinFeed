...
from utils.time_utils import now_bj, bj_str_from_ts, ts_from_bj_str
...
_favorites_cache: str | None = None
_favorites_mtime: float = 0
...
def _load_template(cache_attr: str, mtime_attr: str, filename: str) -> str:
    global _template_cache, _template_mtime, _dashboard_cache, _dashboard_mtime
    global _about_cache, _about_mtime, _sentiment_cache, _sentiment_mtime
    global _favorites_cache, _favorites_mtime

    cache_map = {
        "index": ("_template_cache", "_template_mtime"),
        "dashboard": ("_dashboard_cache", "_dashboard_mtime"),
        "about": ("_about_cache", "_about_mtime"),
        "sentiment": ("_sentiment_cache", "_sentiment_mtime"),
        "favorites": ("_favorites_cache", "_favorites_mtime"),
    }
    cache_name, mtime_name = cache_map.get(cache_attr, ("_template_cache", "_template_mtime"))
    ...
    cache_var = globals()[cache_name]
    mtime_var = globals()[mtime_name]

    with _template_lock:
        if cache_var is not None and current_mtime <= mtime_var:
            return cache_var
    ...
    try:
        with open(template_path, "r", encoding="utf-8") as f:
            content = f.read()
        with _template_lock:
            globals()[cache_name] = content
            globals()[mtime_name] = current_mtime
            return content
    except Exception as e:
        logger.warning(f"加载模板 {filename} 失败: {e}")
        with _template_lock:
            if globals()[cache_name] is None:
                globals()[cache_name] = f"<h1>{filename} not found</h1>"
            return globals()[cache_name]


def _get_template() -> str:
    return _load_template("index", "index", "index.html")

def _get_dashboard_html() -> str:
    return _load_template("dashboard", "dashboard", "dashboard.html")

def _get_about_html() -> str:
    return _load_template("about", "about", "about.html")

def _get_sentiment_html() -> str:
    return _load_template("sentiment", "sentiment", "sentiment.html")

def _get_favorites_html() -> str:
    return _load_template("favorites", "favorites", "favorites.html")
...
def _build_news_response(news_items: list, total: int, page: int, page_size: int, source: str = "all") -> dict:
...
class _WebHandler(BaseHTTPRequestHandler):
    ...
    def _serve_news(self):
        try:
            ...
            stats = db_get_statistics()
            has_more = len(news_items) >= page_size
            next_offset = current_offset + len(news_items) if has_more else None
    ...
    def _serve_favorites_api(self):
        try:
            ...
            result = {
                ...
                "total": total,
                "offset": params["offset"],
                "next_offset": params["offset"] + len(news_items) if has_more else None,
                "limit": page_size,
                "returned_count": len(news_items),
                "has_more": has_more,
                "stats": stats,
