...
| `REFRESH_INTERVAL` | 8 | 主数据轮询周期（秒）；**交易时段**建议 5~15 |
| `IDLE_REFRESH_INTERVAL` | 300 | 非交易时段（午休/收盘后/非交易日）轮询间隔（秒）；行情静止，降为空转以节省 TDX 连接与上游压力；设 0 关闭后台刷新（仅手动触发） |
| `ROTATION_SESSION_ONLY` | 1 | 板块轮动趋势/热力图是否仅在交易时段内采样统计（1=是）；非交易时段视图定格于最近一个交易时点。置 0 可恢复全时段采样（仅供调试） |
| `DETAIL_REFRESH_EVERY` | 30 | 个股资金流详情补全周期（秒，逐股查询成本高）；非交易时段自动跳过 |
...
- **定时轮询**：后台 `RefreshWorker` 线程按交易时段切换轮询间隔——交易时段按
  `REFRESH_INTERVAL` 秒采集，非交易时段按 `IDLE_REFRESH_INTERVAL` 空转；
- **实时更新**：前端每 5 秒拉取各 API 渲染，图表增量更新；
- **轮动采样门控**：板块轮动趋势与资金轮动热力图**只在交易时段内采样统计**
  （见 `session.py`）。收盘后 / 午间休市行情静止，继续采样只会写入重复点、使趋势
  退化为直线并挤占 `HISTORY_LEN` 历史窗口，故 `record_history=False` 跳过采样；
  这两个视图定格于最近一个交易时点，面板以徽标标注时段与数据截止时间；
...
**Q: 非交易时段有数据吗？**
A: 有。榜单、信号、异动等面板显示最近一个交易日的收盘快照；板块轮动趋势与资金
轮动热力图**仅在交易时段采样**，非交易时段定格于最近一个交易时点（面板徽标显示
时段与数据截止时间），开盘后自动从当前时点继续采样。如需全时段采样（调试/离线
回放），将 `ROTATION_SESSION_ONLY=0`。
